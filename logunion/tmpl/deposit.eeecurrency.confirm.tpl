...
<form name=spend method=post action="https://eeecurrency.com/cgi-bin/api.cgi">
  Your eeeCurrency account number <b>{%$account%}</b><br>
...
<INPUT type="hidden" name="SELLERACCOUNTID" value="{%$settings.def_payee_account_eeecurrency%}" >
<input type="hidden" name="POSTURLNUM" value="{%$settings.eeecurrency_posturl%}">
<INPUT type="hidden" name="RETURNURL" value="{%$settings.site_url%}/return_egold&process=yes">
<INPUT type="hidden" name="CANCEL_RETURN" value="{%$settings.site_url%}/return_egold&process=no">
