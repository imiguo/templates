...
<form name=spend method=post>
    Amount ({%$currency_sign%}): <b>{%$amount%}</b> of {%$ps%}
    <br> Deposit from an account balance.
    <br>
    {%if $use_compound%}
            ...
            <tr>
                ...
                <td>
                    <select name='compound' class=inpts>
                        ...
                        <option value="{%$compound_percents[p].percent%}">{%$compound_percents[p].percent%}%</option>{%/section%}
                    </select>
                </td>
                {%else%}
                <td width=99%>
                    <input type=text name='compound' value="{%$compound_min_percents%}" class=inpts size=5>
                </td>
            </tr>
            <tr>
                <td nowrap colspan=2>
                (You can set any percent between
                    <b>{%$compound_min_percents%}%</b> and <b>{%$compound_max_percents%}%</b>)
                </td>
                {%/if%}
            </tr>
...
{%else%}
    ...
    {%if $on_hold == 1%}
        ...
        <br>
    {%/if%}
    <br> Click <a href="?a=deposit">here</a> and try again.
{%/if%}
