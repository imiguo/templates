...
<br>
<br>
Amount ({$currency_sign}): <b>{$amount}</b>
of {$cname}
<br>
    ...
    {else}
      <table cellspacing=0 cellpadding=2 border=0>
        <tr>
          ...
          {else}
          <td width=99%>
            <input type=text name='compound' value="{$compound_min_percents}" class=inpts size=5></td>
        </tr>
        <tr>
          <td nowrap colspan=2>
            (You can set any percent between <b>{$compound_min_percents}%</b>
            and
        ...
        <td colspan=2>
          <small>
            Example: {$compounding}% of your earning will be accumulate on deposit.
          </small>
        </td>
      </tr-->
      </table>
...
<table cellspacing=0 cellpadding=2 border=0>
  ...
  <tr>
    <td>{$fields[f].name}</td>
    <td>
      <input type="text" name="fields[{$fields[f].id}]" value="" class=inpts></td>
  </tr>
  {/section}
</table>
{/if}
...
&nbsp;
<input type=button class=sbmt value="Cancel" onclick="document.location='?a=deposit'"><input type="hidden" name="_token" value="{$csrf_token}"></form>
{else}
...
{if $not_enough_funds == 1} You have not enough funds to complete the operation.
<br>
{/if}
{if $less_than_min == 1}
