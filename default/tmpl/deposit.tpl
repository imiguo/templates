...
{if $frm.say eq 'deposit_saved'}
    <h3>The deposit has been saved. It will became active when the administrator checks statistics.</h3>
    <br>
    <br>
{/if}
    ...
    {section name=plans loop=$plans}
        <table cellspacing=1 cellpadding=2 border=0 width=100%>
            ...
            {if $settings.enable_calculator}
                <tr>
                    <td colspan=3 align=right><a href="javascript:openCalculator('{$plans[plans].id}')">Calculate your
                            profit &gt;&gt;</a></td>
                </tr>
            {/if}
        </table>
            ...
            <td align=right>
                <small>
                    {section name=p loop=$ps}
                        {if $ps[p].balance > 0}${$ps[p].balance} of {$ps[p].name}{if $hold[p].amount > 0} / ${$hold[p].amount} on hold{/if}
                            <br>
                        {/if}
                    {/section}
                </small>
