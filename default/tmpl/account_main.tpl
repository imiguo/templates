...
<tr>
 ...
 <td>$<b>{$ab_formated.total}</b><br>
 <small>
{section name=p loop=$ps}
   {if $ps[p].balance > 0}${$ps[p].balance} of {$ps[p].name}<br>{/if}
{/section}
</tr><tr>
    <td>Earned Total:</td>
 <td>$<b>{$ab_formated.earning}</b></td>
</tr><tr>
 <td>Active Deposit:</td>
 <td>$<b>{$ab_formated.active_deposit}</b></td>
</tr><tr>
 <td>&nbsp;</td>
</tr>
{if $last_deposit}
<tr>
 <td>Last Deposit:</td>
 <td>$<b>{$last_deposit}</b> &nbsp; <small>{$last_deposit_date}</small></td>
</tr>
{/if}
{if $ab_formated.deposit != 0}
<tr>
 <td>Total Deposit:</td>
 <td>$<b>{$ab_formated.deposit}</b></td>
</tr>
{/if}
{if $last_withdrawal}
<tr>
 <td>Last Withdrawal:</td>
 <td>$<b>{$last_withdrawal}</b> &nbsp; <small>{$last_withdrawal_date}</small></td>
</tr>
{/if}
{if $ab_formated.withdrawal > 0}
<tr>
    <td>Withdrawn Total:</td>
 <td>$<b>{$ab_formated.withdrawal}</b></td>
</tr>
{/if}
<tr>
 <td>&nbsp;</td>
...
{section name=p loop=$ps}
  {if $ps[p].pending_col > 0}{$ps[p].pending_col} {$ps[p].name} deposit{if $ps[p].pending_col > 1}s{/if} of ${$ps[p].pending_amount} total pending<br>{/if}
{/section}
