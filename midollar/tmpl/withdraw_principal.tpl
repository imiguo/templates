...
<table cellspacing=0 cellpadding=2 border=0>
  <tr>
    <td colspan=2>
      Withdrawal from <b>{$currency_sign}{$deposit.deposit}</b>
      deposit from the <b>{$type.name|escape:html}</b>
    </td>
  </tr>
  <tr>
    <td colspan=2>
      Withdrawal fee is
      <b>{$type.withdraw_principal_percent}% or {$currency_sign}0.01 as minimal</b> </b> 
  </td>
</tr>
<tr>
  <td width=1% nowrap>Withdrawal ({$currency_sign}):</td>
  <td>
    <input type=text name=amount value="{$deposit.deposit}" class=inpts size=15 onchange="withdraw()" onkeyup="withdraw()"
...
<tr>
  <td>&nbsp;</td>
  <td>
    <input type=submit value="Withdraw" class=sbmt></td>
</tr>
</table>
</form>
